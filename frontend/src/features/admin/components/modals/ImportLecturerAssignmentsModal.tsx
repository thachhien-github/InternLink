...
import type { LecturerAssignmentImportResultDto } from "../../../../types/api";
...
export const ImportLecturerAssignmentsModal = ({
  isOpen,
  onClose,
  onShowToast,
  onSuccess,
  ...
  const [result, setResult] = useState<LecturerAssignmentImportResultDto | null>(null);
  ...
  const handleDownloadTemplate = async () => {
    try {
      await adminAssignmentsService.downloadLecturerAssignmentTemplate();
      onShowToast("Đã tải xuống file mẫu phân công giảng viên (.xlsx)");
    } catch (err) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setResult(null);
    try {
      const res = await adminAssignmentsService.importLecturerAssignments(
        selectedFile,
        currentSemesterId ?? undefined,
      );
      setResult(res);
      if (res.successCount > 0) {
        onShowToast(`Phân công thành công ${res.successCount}/${res.totalRows} sinh viên cho giảng viên!`);
        onSuccess();
      } else {
        onShowToast(`Import hoàn tất với ${res.failedCount} lỗi.`);
      }
    } catch (err) {
      onShowToast(getApiErrorMessage(err));
    } finally {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-base">
                Import Phân Công Giảng Viên Hướng Dẫn
              </h3>
              <p className="text-xs text-slate-500 font-medium">
                Tải lên file Excel theo mẫu: STT, MSSV, Họ Tên, Lớp, Mã Giảng Viên, Tên Giảng Viên
              </p>
            </div>
          </div>
          <button
          ...
          <div className="p-3 bg-blue-50/70 rounded-md border border-blue-200/80 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-700">
                File mẫu chuẩn phân công giảng viên (.xlsx)
              </span>
            </div>
            <button
              type="button"
              onClick={handleDownloadTemplate}
              className="text-blue-600 hover:text-blue-800 font-bold flex items-center gap-1 bg-white px-3 py-1.5 rounded border border-blue-200 shadow-xs"
            >
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                Định dạng hỗ trợ: .xlsx, .xls • Tự động gắn sinh viên với giảng viên tương ứng
              </p>
            </div>
          </div>
            ...
            <div className="space-y-3 pt-2">
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div className="p-2.5 bg-rose-50 border border-rose-200 rounded-md">
                  <p className="text-[10px] text-rose-700 font-semibold">Lỗi / Bỏ qua</p>
                  <p className="text-base font-bold text-rose-800 flex items-center justify-center gap-1">
                    <AlertTriangle className="w-4 h-4" /> {result.failedCount}
                  </p>
                </div>
              </div>
                    ...
                    {result.errors.map((err, idx) => (
                      <div key={idx} className="text-[11px] text-rose-800 bg-white p-2 rounded border border-rose-200/60 flex items-start gap-2">
                        ...
                        <span className="flex-1">
                          {err.studentName ? <strong>{err.studentName}: </strong> : null}
                          {err.message}
                        </span>
                      </div>
                    ))}
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100">
            ...
            <div className="flex items-center gap-2.5">
              ...
              >
                ...
                <span>
                  {isUploading ? "Đang xử lý import..." : "Bắt đầu Phân công"}
                </span>
              </button>
            </div>
          </div>
