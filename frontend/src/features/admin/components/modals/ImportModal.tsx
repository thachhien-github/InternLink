...
export const ImportModal = ({ isOpen, type, onClose, onShowToast }) => {
  const [fileName, setFileName] = useState(null);
  const [isUploading, setIsUploading] = useState(false);
  if (!isOpen) return null;
  const isLecturer = type === "lecturers";
  const title = isLecturer
    ? "Import Gi\u1EA3ng vi\xEAn t\u1EEB Excel"
    : "Import Sinh vi\xEAn t\u1EEB Excel";
  const handleSimulatedUpload = (e) => {
    ...
    setTimeout(() => {
      ...
      onShowToast(
        `\u0110\xE3 import th\xE0nh c\xF4ng d\u1EEF li\u1EC7u ${isLecturer ? "42 Gi\u1EA3ng vi\xEAn" : "120 Sinh vi\xEAn"} v\xE0o h\u1EC7 th\u1ED1ng!`,
      );
      onClose();
    }, 1e3);
  };
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-md max-w-md w-full p-6 space-y-5 animate-in zoom-in-95">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                Đồng bộ danh sách theo mẫu file chuẩn (.xlsx, .csv)
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSimulatedUpload} className="space-y-4 text-xs">
          ...
          <div className="p-3 bg-slate-50 rounded-md border border-slate-200/80 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-700">
                File mẫu {isLecturer ? "Gi\u1EA3ng vi\xEAn" : "Sinh vi\xEAn"}{" "}
                chuẩn
              </span>
            </div>
            <button
              ...
              onClick={() =>
                onShowToast(
                  `\u0110\xE3 t\u1EA3i file m\u1EABu Mau_Import_${isLecturer ? "GiangVien" : "SinhVien"}.xlsx`,
                )
              }
          ...
          <div
            onClick={() =>
              setFileName(
                isLecturer ? "DS_GiangVien_2026.xlsx" : "DS_SinhVien_K20.xlsx",
              )
            }
          ...
          >
            ...
            <div>
              <p className="font-bold text-slate-800 text-xs">
                {fileName
                  ? fileName
                  : "B\u1EA5m \u0111\u1EC3 ch\u1ECDn file ho\u1EB7c k\xE9o th\u1EA3 v\xE0o \u0111\xE2y"}
              </p>
              <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                Hỗ trợ các định dạng .XLSX, .XLS, .CSV (Tối đa 20MB)
              </p>
            </div>
          </div>
