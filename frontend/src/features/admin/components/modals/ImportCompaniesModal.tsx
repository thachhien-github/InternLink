...
import type { CompanyImportResultDto } from "../../../../types/api";
...
export const ImportCompaniesModal = ({
  isOpen,
  onClose,
  onShowToast,
  onSuccess,
  ...
  const handleDownloadTemplate = async () => {
    try {
      await adminCompaniesService.downloadImportTemplate();
      onShowToast("Đã tải xuống file mẫu danh sách doanh nghiệp (.xlsx)");
    } catch (err) {
          ...
          <div className="p-3 bg-blue-50/70 rounded-md border border-blue-200/80 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-700">
                File mẫu chuẩn danh sách doanh nghiệp (Mau-danh-sach-doanh-nghiep.xlsx)
              </span>
            </div>
            <button
              type="button"
