...
import type { CompanyAllocationImportResultDto } from "../../../../types/api";
...
export const ImportCompanyAllocationsModal = ({
  isOpen,
  onClose,
  onShowToast,
  onSuccess,
  ...
  const [result, setResult] = useState<CompanyAllocationImportResultDto | null>(null);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setResult(null);
    try {
      const res = await adminAssignmentsService.importCompanyAllocations(
        selectedFile,
        currentSemesterId ?? undefined,
      );
      setResult(res);
      if (res.successCount > 0) {
        onShowToast(`Phân bổ thành công ${res.successCount}/${res.totalRows} sinh viên vào doanh nghiệp!`);
        onSuccess();
      } else {
        onShowToast(`Import hoàn tất với ${res.failedCount} lỗi. Vui lòng kiểm tra chi tiết.`);
      }
    } catch (err) {
      onShowToast(getApiErrorMessage(err));
    } finally {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-base">
                Import Phân Bổ Sinh Viên vào Doanh Nghiệp
              </h3>
              <p className="text-xs text-slate-500 font-medium">
                Tải lên file Excel theo mẫu: STT, Họ Tên, Lớp, Công Ty Thực Tập (hoặc MSSV)
              </p>
            </div>
          </div>
          <button
          ...
          <div className="p-3 bg-blue-50/70 rounded-md border border-blue-200/80 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-700">
                File mẫu chuẩn phân bổ doanh nghiệp (.xlsx)
              </span>
            </div>
            <button
              type="button"
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                Định dạng hỗ trợ: .xlsx, .xls • Khớp tự động theo Họ Tên + Lớp hoặc MSSV
              </p>
            </div>
          </div>
                    ...
                    {result.errors.map((err, idx) => (
                      <div key={idx} className="text-[11px] text-rose-800 bg-white p-2 rounded border border-rose-200/60 flex items-start gap-2">
                        ...
                        <span className="flex-1">
                          {err.studentName ? <strong>{err.studentName} ({err.className || "-"}): </strong> : null}
                          {err.message}
                        </span>
                      </div>
                    ))}
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100">
            ...
            <div className="flex items-center gap-2.5">
              ...
              >
                ...
                <span>
                  {isUploading ? "Đang xử lý import..." : "Bắt đầu Phân bổ"}
                </span>
              </button>
            </div>
          </div>
