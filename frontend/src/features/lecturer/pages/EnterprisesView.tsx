...
export const EnterprisesView = ({
  ...
  const filteredEnterprises = useMemo(() => {
    return enterprises.filter((item) => {
      ...
      const matchField =
        fieldFilter === "T\u1EA5t c\u1EA3" ||
        (item.field && item.field.includes(fieldFilter));
      const matchLocation =
      ...
      return (
        matchSearch && matchField && matchLocation && matchStatus && matchHiring
      );
    });
  }, [
    enterprises,
  ...
  const cooperatingCount = enterprises.filter(
    (e) =>
      e.status === "\u0110ang h\u1EE3p t\xE1c" ||
      e.status === "\u0110\u1ED1i t\xE1c \u01B0u ti\xEAn" ||
      e.status === "\u0110\xE3 duy\u1EC7t",
  ).length;
  const hiringCount = enterprises.filter((e) => e.isHiring).length;
  const priorityCount = enterprises.filter(
    (e) =>
      e.isPriority ||
      e.status === "\u0110\u1ED1i t\xE1c \u01B0u ti\xEAn" ||
      e.badge === "\u0110\u1ED0I T\xC1C \u01AFU TI\xCAN",
  ).length;
  const newCount = enterprises.filter(
    (e) => e.status === "Ch\u1EDD duy\u1EC7t",
  ).length;
  ...
  const handleCreateSubmit = (e) => {
    ...
    const created = {
      ...
      shortCode:
        newCompany.shortCode || newCompany.name.substring(0, 3).toUpperCase(),
      ...
      badge: newCompany.isPriority
        ? "\u0110\u1ED0I T\xC1C \u01AFU TI\xCAN"
        : "CH\xCDNH TH\u1EE8C",
    ...
    triggerToast(
      `\u0110\xE3 th\xEAm th\xE0nh c\xF4ng doanh nghi\u1EC7p ${created.name}`,
    );
    setNewCompany({
      name: "",
      shortCode: "",
  ...
  if (!readOnly && (isFormWorkspaceOpen || companyToEdit)) {
    return (
      <CompanyFormWorkspace
        ...
        onSave={(data, isDraft) => {
          ...
          triggerToast(
            isDraft
              ? `\u0110\xE3 l\u01B0u nh\xE1p doanh nghi\u1EC7p ${data.name}`
              : `\u0110\xE3 l\u01B0u th\xF4ng tin doanh nghi\u1EC7p ${data.name}`,
          );
        }}
      />
    );
  ...
  if (selectedCompanyDetail) {
    return (
      <CompanyDetailWorkspace
        ...
        onAssignStudents={(comp) =>
          triggerToast(
            `\u0110\xE3 m\u1EDF giao di\u1EC7n ph\xE2n c\xF4ng sinh vi\xEAn cho ${comp.shortCode}`,
          )
        }
      />
    );
  ...
  return (
    <div className="space-y-5 max-w-[1500px] mx-auto animate-in fade-in duration-200 pb-10">
      ...
      <PageHeader
        ...
        subtitle={
          readOnly
            ? "Tra cứu đối tác thực tập (chỉ xem). Gán DN trên hồ sơ sinh viên."
            ...
            ? [
                {
                  ...
                  icon: Download,
                  onClick: () =>
                    triggerToast("Đã xuất danh mục doanh nghiệp (.xlsx)"),
                  variant: "secondary",
                },
              ]
            : [
                {
                  ...
                  onClick: () =>
                    triggerToast(
                      "Đã nhập danh sách 15 doanh nghiệp từ file Excel",
                    ),
                  variant: "secondary",
                },
                {
                  ...
                  onClick: () =>
                    triggerToast(
                      "Đã xuất báo cáo đối tác doanh nghiệp (.xlsx)",
                    ),
                  variant: "secondary",
                },
                {
      ...
      <Toolbar
        left={
          <p className="text-xs text-slate-500 font-medium">
            <span className="font-bold text-slate-800">{totalCount}</span> DN ·{" "}
            <span className="font-bold text-emerald-700">{cooperatingCount}</span>{" "}
            hợp tác ·{" "}
            <span className="font-bold text-sky-700">{hiringCount}</span> đang
            tuyển
          </p>
        }
      />
      ...
      <div className="w-full space-y-4">
        {filteredEnterprises.length === 0 ? (
          /* EMPTY STATE */
          <div className="bg-white p-12 rounded-lg border border-slate-200/80 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                {readOnly
                  ...
                  : "Hãy thêm doanh nghiệp đầu tiên hoặc điều chỉnh bộ lọc."}
              </p>
            </div>
            {!readOnly && (
              <button
                onClick={() => setShowAddModal(true)}
                className="px-4 py-2 bg-blue-600 text-white font-bold text-xs rounded-md hover:bg-blue-700 transition-colors inline-flex items-center gap-1.5 cursor-pointer"
              >
                    ...
                    >
                      ...
                      <td className="p-3.5 pl-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-md bg-blue-50 text-blue-700 font-bold text-xs flex items-center justify-center border border-blue-100 shrink-0">
                            {item.shortCode}
                          </div>
                          <div>
                            <span
                              className="font-bold text-slate-900 text-xs block hover:text-blue-600 cursor-pointer"
                      ...
                      <td className="p-3.5">
                        <div className="flex flex-wrap gap-1 max-w-[180px]">
                          {item.openPositions &&
                          item.openPositions.length > 0 ? (
                            item.openPositions.slice(0, 2).map((pos, pIdx) => (
                              <span
                                key={pIdx}
                      ...
                      <td className="p-3.5 text-center">
                        <span className="font-bold text-blue-600 font-mono text-xs px-2 py-0.5 bg-blue-50 rounded-lg border border-blue-100">
                      ...
                      <td className="p-3.5">
                        <span
                          className={`px-2.5 py-1 text-[10px] font-bold rounded-full inline-flex items-center gap-1 ${item.status === "\u0110\u1ED1i t\xE1c \u01B0u ti\xEAn" || item.isPriority ? "bg-amber-100 text-amber-900 border border-amber-300" : item.status === "\u0110ang h\u1EE3p t\xE1c" || item.status === "\u0110\xE3 duy\u1EC7t" ? "bg-emerald-100 text-emerald-800 border border-emerald-300" : item.status === "Chờ duyệt" ? "bg-amber-50 text-amber-800 border border-amber-200" : "bg-rose-100 text-rose-800 border border-rose-300"}`}
                        >
                              ...
                              {item.status === "Ch\u1EDD duy\u1EC7t" && (
                                <button
                                  onClick={() => {
                                    ...
                                    triggerToast(
                                      `Đã phê duyệt đối tác ${item.name}`,
                                    );
                                  }}
                                  className="px-2 py-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] rounded-lg transition-colors ml-1 cursor-pointer"
            ...
            <div className="p-3.5 bg-slate-50/80 border-t border-slate-200/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2 font-semibold text-slate-600">
                ...
                <span className="text-slate-400 font-normal">
                  ...
                  -{" "}
                  {Math.min(currentPage * pageSize, filteredEnterprises.length)}{" "}
                  / tổng {filteredEnterprises.length} doanh nghiệp)
                </span>
              </div>
          ...
          {enterprises
            ...
            .map((comp, idx) => (
              <div
                ...
                className="p-3 bg-slate-50 hover:bg-blue-50/60 rounded-md border border-slate-200/80 transition-colors cursor-pointer flex items-center justify-between"
              >
                <div className="flex items-center gap-2.5 truncate">
                  <span className="font-bold text-blue-600 bg-blue-100/80 px-2 py-0.5 rounded-md text-xs">
                    #{idx + 1}
                  </span>
                  <div className="truncate">
                    ...
                    <span className="text-[10px] text-slate-500">
                      {comp.studentCount} SV đang thực tập
                    </span>
                  </div>
                </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-lg w-full p-6 shadow-md border border-slate-200 space-y-4 animate-in zoom-in-95">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-3 text-xs">
              ...
              <div className="flex items-center justify-end gap-2 pt-4 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setShowAddModal(false)}
                  className="px-4 py-2 bg-slate-100 text-slate-700 font-bold rounded-md hover:bg-slate-200"
                >
                  Hủy
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-md shadow-md"
                >
                  Lưu &amp; Thêm đối tác
                </button>
              </div>
      ...
      {detailModalItem && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full p-6 shadow-md border border-slate-200 space-y-5 animate-in zoom-in-95">
            <div className="flex items-start justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-md bg-blue-600 text-white font-bold text-base flex items-center justify-center shadow-md">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">
                    {detailModalItem.field || "C\xF4ng ngh\u1EC7 Th\xF4ng tin"}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div className="p-3 bg-slate-50 rounded-md space-y-1">
                ...
                <p className="font-bold text-slate-900">
                  {detailModalItem.contactPerson ||
                    "Tr\u1EA7n Th\u1ECB Thu H\xE0"}
                </p>
                <p className="text-slate-500 font-mono">
                  {detailModalItem.contactPhone || "024 7300 7300"}
                </p>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-md space-y-1">
                ...
                <p className="font-bold text-blue-600">
                  {detailModalItem.contactEmail}
                </p>
                <p className="text-slate-500">
                  {detailModalItem.website || "https://fpt-software.com"}
                </p>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-md space-y-1">
                ...
                <p className="font-bold text-slate-900">
                  ...
                  <span className="text-blue-600 font-bold">
                    {detailModalItem.studentCount} SV
                  </span>{" "}
                  / Tối đa {detailModalItem.capacity || 50} SV
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <span className="text-xs font-bold text-slate-900 uppercase">
                Vị trí tuyển thực tập sinh mở rộng
              </span>
              <div className="flex flex-wrap gap-2">
                {detailModalItem.openPositions?.map((pos, pIdx) => (
                  <span
                    key={pIdx}
                    className="px-2.5 py-1 bg-blue-50 text-blue-800 font-bold text-xs rounded-md border border-blue-200"
                  >
                    {pos}
                  </span>
                )) || (
                  <span className="text-xs text-slate-500">
                    Mở toàn bộ vị trí Software Engineering
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
              <button
                onClick={() => setDetailModalItem(null)}
                className="px-4 py-2 bg-slate-100 text-slate-700 font-bold text-xs rounded-md hover:bg-slate-200"
              >
                Đóng
              </button>
            </div>
      ...
      {statsModalItem && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-md border border-slate-200 space-y-4 animate-in zoom-in-95">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <BarChart2 className="w-5 h-5 text-emerald-600" />
                <h3 className="font-bold text-base text-slate-900">
                  Thống kê tiếp nhận {statsModalItem.shortCode}
                </h3>
              </div>
              <button
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-emerald-50 rounded-md border border-emerald-200 text-emerald-900 space-y-1">
                <span className="text-[10px] font-bold uppercase">
                  Đánh giá trung bình từ Sinh viên &amp; Giảng viên
                </span>
                <div className="flex items-center gap-2">
                  <span className="text-2xl font-bold">
                    {statsModalItem.rating || 4.8} / 5.0
                  </span>
