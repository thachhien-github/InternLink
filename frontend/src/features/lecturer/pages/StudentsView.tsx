...
export const StudentsView = ({
  students,
  enterprises = [] as import("../../../types/enterprise").Enterprise[],
  onSendReminder,
  onAddStudent,
}: {
    ...
    return students
      .filter((s) => {
        ...
        const matchesLecturer =
          lecturerFilter === "T\u1EA5t c\u1EA3" ||
          s.lecturer === lecturerFilter;
        const matchesCompany =
          companyFilter === "T\u1EA5t c\u1EA3" ||
          (companyFilter === "Ch\u01B0a c\xF3"
            ? s.company === "Ch\u01B0a c\xF3"
            : s.company === companyFilter);
        const matchesStatus =
          statusFilter === "T\u1EA5t c\u1EA3" || s.status === statusFilter;
        const matchesMajor =
          majorFilter === "T\u1EA5t c\u1EA3" || s.major === majorFilter;
        let matchesProgress = true;
        if (progressFilter === "< 30%") matchesProgress = s.progress < 30;
        else if (progressFilter === "30% - 70%")
          matchesProgress = s.progress >= 30 && s.progress <= 70;
        else if (progressFilter === "> 70%")
          matchesProgress = s.progress > 70 && s.progress < 100;
        else if (progressFilter === "100%")
          matchesProgress = s.progress === 100;
        let matchesGpa = true;
        if (gpaFilter === "GPA \u2265 3.6") matchesGpa = s.gpa >= 3.6;
        else if (gpaFilter === "3.2 - 3.59")
          matchesGpa = s.gpa >= 3.2 && s.gpa < 3.6;
        else if (gpaFilter === "2.8 - 3.19")
          matchesGpa = s.gpa >= 2.8 && s.gpa < 3.2;
  ...
  const handleBulkStatusChange = () => {
    showToast(
      `\u0110\xE3 chuy\u1EC3n tr\u1EA1ng th\xE1i cho ${selectedIds.length} sinh vi\xEAn sang "\u0110ang ch\u1EC9nh s\u1EEDa"`,
    );
    setSelectedIds([]);
  };
  const handleCreateStudentSubmit = (e) => {
    ...
    onAddStudent?.({
      ...
      chatCount: 0,
      lastReportName:
        "\u0110\u0103ng k\xFD \u0111\u1EC1 t\xE0i th\u1EF1c t\u1EADp",
      lastReportDate: "H\xF4m nay",
      updatedAt: "H\xF4m nay",
    });
    setShowAddModal(false);
    showToast(
      `\u0110\xE3 th\xEAm th\xE0nh c\xF4ng sinh vi\xEAn ${newStudent.name}`,
    );
    setNewStudent({
      name: "",
      mssv: "",
  ...
  const handleGenerateAiComment = () => {
    ...
    setTimeout(() => {
      if (commentStudent) {
        setCommentText(
          `[AI \u0110\u1EC1 xu\u1EA5t Nh\u1EADn x\xE9t]: Sinh vi\xEAn ${commentStudent.name} (MSSV: ${commentStudent.mssv}) \u0111\xE3 ho\xE0n th\xE0nh ${commentStudent.progress}% kh\u1ED1i l\u01B0\u1EE3ng th\u1EF1c t\u1EADp t\u1EA1i ${commentStudent.company}. B\xE1o c\xE1o g\u1EA7n nh\u1EA5t "${commentStudent.lastReportName}" c\xF3 k\u1EBFt qu\u1EA3 \u0111\u1EA1t ti\xEAu chu\u1EA9n. \u0110\u1EC1 xu\u1EA5t ti\u1EBFp t\u1EE5c duy tr\xEC ti\u1EBFn \u0111\u1ED9 v\xE0 ho\xE0n thi\u1EC7n b\xE1o c\xE1o t\u1ED5ng k\u1EBFt.`,
        );
      }
      setAiDrafting(false);
    }, 800);
  ...
  const handleSendChatMessage = (e) => {
    ...
    setChatHistory((prev) => [
      ...prev,
      {
        sender: "TS. Ph\u1EA1m Minh Anh (Gi\u1EA3ng vi\xEAn)",
        text: chatMessage,
        time: "V\u1EEBa xong",
        role: "lecturer",
      },
  ...
  const renderStatusChip = (status, riskFlag) => {
    switch (status) {
      ...
      case "\u0110\xFAng h\u1EA1n":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200/80 shrink-0">
      ...
      case "Ch\u1EADm":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-blue-50 text-blue-700 border border-blue-200/80 shrink-0">
      ...
      case "Qu\xE1 h\u1EA1n":
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-rose-50 text-rose-700 border border-rose-200/80 shrink-0">
  ...
  if (detailStudent) {
    return (
      <StudentDetailWorkspace
        ...
        onAssignCompany={(companyName) => {
          setDetailStudent((prev) =>
            prev ? { ...prev, company: companyName } : prev,
          );
          showToast(`Đã gán doanh nghiệp ${companyName} cho ${detailStudent.name}`);
        }}
        onSendComment={(comment) =>
          showToast(`Đã lưu nhận xét cho ${detailStudent.name}`)
        }
        onChat={() =>
  ...
  return (
    <div className="space-y-5 max-w-[1500px] mx-auto animate-in fade-in duration-200">
      ...
      <PageHeader
        ...
        subtitle="Chỉ xem sinh viên thuộc nhóm hướng dẫn — mở hồ sơ để theo dõi thực tập và gán doanh nghiệp."
        actions={[
          {
            label: "Xuất Excel",
            icon: Download,
            onClick: () =>
              showToast(
                "Đã xuất danh sách sinh viên được phân công (.xlsx)",
              ),
            variant: "secondary",
          },
        ]}
        ...
        left={
          <p className="text-xs text-slate-500 font-medium">
            ...
            <span className="font-bold text-emerald-700">
              {students.filter((s) => s.company !== "Chưa có").length}
            </span>{" "}
            đã có DN ·{" "}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-2 text-xs">
          ...
          <div>
            <select
              ...
              className="w-full p-1.5 bg-blue-50/80 border border-blue-200 rounded-md outline-none font-bold text-blue-900 text-[11px]"
            >
      ...
      {selectedIds.length > 0 && (
        <div className="bg-blue-900 text-white p-3 rounded-lg shadow-sm flex flex-wrap items-center justify-between gap-3 animate-in fade-in zoom-in-98">
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs">
            <button
              ...
              className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-md flex items-center gap-1.5 transition-colors"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 bg-blue-800 hover:bg-blue-700 text-white font-bold rounded-md flex items-center gap-1.5 transition-colors"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-md flex items-center gap-1.5 transition-colors"
            >
      ...
      <div className="space-y-4">
        <div className="bg-white rounded-lg border border-slate-200/80 shadow-xs overflow-hidden w-full">
          {filteredStudents.length === 0 ? (
            ...
            <div className="p-12 text-center space-y-4">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-base">
                  Chưa có sinh viên trong học kỳ này.
                </h3>
                <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                  Không tìm thấy sinh viên tương ứng với bộ lọc hiện tại.
                  Thử xóa bộ lọc hoặc liên hệ Ban Điều hành nếu thiếu phân công.
                </p>
              </div>
            </div>
          ) : (
            <div className="w-full">
              <table className="w-full text-left text-xs border-collapse table-auto">
                <thead className="bg-slate-50/90 border-b border-slate-200/80 text-[10px] font-bold uppercase text-slate-400 tracking-wider">
                  <tr>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3">
                          ...
                          <span className="text-[10px] text-blue-600 font-bold ml-1.5">
                            GPA {s.gpa}
                          </span>
                        </td>
                        ...
                        <td className="py-2.5 px-3 max-w-[200px]">
                          <p
                            className={`font-bold text-[11px] truncate ${s.company === "Ch\u01B0a c\xF3" ? "text-slate-400 italic" : "text-slate-900"}`}
                          >
                            {s.company}
                          </p>
                        ...
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2">
                            <span className="text-[11px] font-bold text-slate-800 w-8">
                              {s.progress}%
                            </span>
                        ...
                        <td className="py-2.5 px-3 max-w-[160px]">
                          <p
                            className="text-slate-800 font-semibold text-[11px] truncate"
                            title={s.lastReportName}
                          >
                          ...
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <div className="relative">
                              ...
                              {activeDropdownId === s.id && (
                                <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-md shadow-md border border-slate-200 z-30 py-1 text-xs animate-in fade-in zoom-in-95">
                                  <button
                                    onClick={() => {
                                      onSendReminder?.(s);
                                  ...
                                  <button
                                    onClick={() => {
                                      showToast(
                                        `\u0110\xE3 xu\u1EA5t b\xE1o c\xE1o c\xE1 nh\xE2n c\u1EE7a ${s.name}`,
                                      );
                                      setActiveDropdownId(null);
                                    }}
                                    className="w-full px-3 py-2 text-left hover:bg-slate-50 text-slate-700 font-semibold flex items-center gap-2"
        ...
        <div className="bg-white p-4 rounded-lg border border-slate-200/80 shadow-xs space-y-3">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between items-center p-2 rounded-md bg-slate-50">
      ...
      {detailStudent && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-xl w-full shadow-md border border-slate-200 animate-in fade-in zoom-in-95 space-y-4">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    {renderStatusChip(
                      ...
                      detailStudent.riskFlag,
                    )}
                  </div>
                  <p className="text-slate-500 font-mono">
                    MSSV: {detailStudent.mssv} • Lớp: {detailStudent.class}
                  </p>
                  <p className="text-blue-600 font-semibold">
                    Chuyên ngành: {detailStudent.major}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-slate-50 rounded-md border border-slate-100 space-y-1">
                  ...
                  <p className="font-bold text-slate-900 text-sm">
                    {detailStudent.company}
                  </p>
                  <p className="text-slate-600">
                    Vị trí:{" "}
                    <strong>
                      {detailStudent.position || "Th\u1EF1c t\u1EADp sinh"}
                    </strong>
                  </p>
                  <p className="text-slate-500 text-[11px]">
                    Mentor: {detailStudent.supervisor}
                  </p>
                </div>
                ...
                <div className="p-3 bg-slate-50 rounded-md border border-slate-100 space-y-1">
                  ...
                  <p className="font-bold text-slate-900 text-sm">
                    {detailStudent.lecturer}
                  </p>
                  <p className="text-blue-600 font-bold">
                    Điểm GPA tích lũy: {detailStudent.gpa} / 4.0
                  </p>
                </div>
              </div>

              <div className="p-3 bg-blue-50/60 border border-blue-100 rounded-md space-y-1">
                <div className="flex justify-between font-bold text-blue-900">
                  <span>Tiến độ thực tập: {detailStudent.progress}%</span>
                  <span>
                    Bài nộp gần nhất:{" "}
                    {detailStudent.lastReportDate || "M\u1EDBi nh\u1EA5t"}
                  </span>
                </div>
                <p className="text-slate-700 font-medium">
                  {detailStudent.lastReportName || "Ch\u01B0a n\u1ED9p b\xE0i"}
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
              <button
                onClick={() => setDetailStudent(null)}
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-md text-xs"
              >
                Đóng
              </button>
              <button
                ...
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-md text-xs shadow-xs"
              >
                Gửi nhắc nộp bài
              </button>
            </div>
      ...
      {commentStudent && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <form
            ...
            className="bg-white rounded-lg p-6 max-w-lg w-full shadow-md border border-slate-200 animate-in fade-in zoom-in-95 space-y-4"
          >
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
              ...
              <button
                ...
                className="p-1 text-slate-400 font-bold"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-slate-50 rounded-md flex items-center justify-between border border-slate-100">
                <div>
                  <p className="font-bold text-slate-900">
                    {commentStudent.name} ({commentStudent.mssv})
                  </p>
                  <p className="text-slate-500">
                    {commentStudent.company} • Tiến độ:{" "}
                    {commentStudent.progress}%
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  disabled={aiDrafting}
                  className="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold rounded-lg border border-blue-200 flex items-center gap-1 transition-colors"
                >
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">
                  Nội dung nhận xét của giảng viên
                </label>
                <textarea
                  rows={4}
                  value={commentText}
      ...
      {chatStudent && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-5 max-w-lg w-full shadow-md border border-slate-200 animate-in fade-in zoom-in-95 space-y-3 flex flex-col h-[520px]">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    GV: {chatStudent.lecturer} • SV: {chatStudent.name}
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Chat Body */}
            <div className="flex-1 overflow-y-auto space-y-3 p-2 bg-slate-50/70 rounded-md border border-slate-100 text-xs">
              {chatHistory.map((msg, idx) => (
                <div
                  key={idx}
                  className={`p-3 rounded-md max-w-[85%] ${msg.role === "lecturer" ? "ml-auto bg-blue-600 text-white rounded-br-xs" : "bg-white border border-slate-200 text-slate-800 rounded-bl-xs"}`}
                >
            ...
            >
              <input
                ...
                className="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-xs outline-none focus:border-blue-500 font-medium"
              />
              <button
                type="submit"
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-md text-xs flex items-center gap-1 shadow-xs"
              >
                <Send className="w-3.5 h-3.5" />
                <span>Gửi</span>
              </button>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-md border border-slate-200 space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <select className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-md font-bold text-slate-800 outline-none">
                  <option value="FPT Software">
                    FPT Software (Còn 5 chỉ tiêu)
                  </option>
                  <option value="VNG Corporation">
                    VNG Corp (Còn 3 chỉ tiêu)
                  </option>
                  <option value="Viettel Group">
                    Viettel Group (Còn 4 chỉ tiêu)
                  </option>
                  <option value="CMC Global">
                    CMC Global (Còn 8 chỉ tiêu)
                  </option>
                </select>
              </div>
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">
                  Cán bộ phụ trách (Mentor)
                </label>
                <input
                  type="text"
                  defaultValue="Nguyễn Văn Hải (Lead Tech)"
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-md font-semibold text-slate-800 outline-none"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                onClick={() => {
                  ...
                  showToast(
                    "\u0110\xE3 ho\xE0n t\u1EA5t ph\xE2n c\xF4ng doanh nghi\u1EC7p th\u1EF1c t\u1EADp th\xE0nh c\xF4ng!",
                  );
                  setSelectedIds([]);
                }}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-md text-xs shadow-xs"
