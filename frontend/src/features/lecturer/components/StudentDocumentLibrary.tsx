...
export const StudentDocumentLibrary = ({
  ...
  const categories = ["T\u1EA5t c\u1EA3", "Bi\u1EC3u m\u1EABu", "K\u1EBF ho\u1EA1ch", "Nh\u1EADt k\xFD", "B\xE1o c\xE1o", "H\u01B0\u1EDBng d\u1EABn"];
  ...
  const handleDownload = (doc) => {
    ...
    triggerToast(`\u0110\xE3 t\u1EA3i xu\u1ED1ng th\xE0nh c\xF4ng: ${doc.title}`);
  };
  ...
  return <div className="space-y-6 animate-in fade-in duration-200 pb-16 font-sans">
    ...
    /* HEADER WITH ROLE SWITCHER */
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        {onSwitchToLecturerView && <button
    onClick={onSwitchToLecturerView}
    className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-colors border border-slate-200 flex items-center gap-1.5"
  >
    ...
    className="w-full pl-10 pr-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 focus:bg-white transition-all text-slate-900 font-medium"
  />
        </div>
    ...
    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${selectedCategory === cat ? "bg-blue-600 text-white shadow-sm" : "bg-slate-100 hover:bg-slate-200 text-slate-700"}`}
  >
              {cat === "T\u1EA5t c\u1EA3" ? "\u{1F4C1} T\u1EA5t c\u1EA3" : `\u{1F4C1} ${cat}`}
            </button>)}
        </div>
      </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-extrabold text-slate-900 flex items-center gap-2">
              ...
              <span className="text-xs font-normal text-slate-400">({filteredDocs.length} tệp)</span>
            </h2>
          </div>
    ...
    >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className={`w-9 h-9 rounded-xl border flex items-center justify-center ${badgeInfo.bg}`}>
                        {badgeInfo.icon}
                      </div>
                      <span className="text-[10px] font-black px-2 py-0.5 bg-blue-100 text-blue-800 rounded-md border border-blue-200">
      ...
      className="font-extrabold text-slate-900 text-sm hover:text-blue-600 cursor-pointer line-clamp-2 leading-snug"
    >
                      {doc.title}
                    </h3>

                    <p className="text-xs text-slate-500 font-medium line-clamp-2">
                      {doc.description || "T\xE0i li\u1EC7u h\u01B0\u1EDBng d\u1EABn ch\xEDnh th\u1EE9c t\u1EEB Gi\u1EA3ng vi\xEAn."}
                    </p>
                  </div>

                  <div className="pt-3 border-t border-slate-100 space-y-2">
                    <div className="flex items-center justify-between text-[10px] text-slate-400 font-medium">
                      <span>Cập nhật: <strong className="text-slate-700">{doc.updatedAt}</strong></span>
    ...
    /* RECENT DOWNLOADS SIDEBAR (4 cols) */
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs space-y-3">
            ...
            {recentDownloads.length === 0 ? <p className="text-xs text-slate-400 italic">Bạn chưa tải xuống tài liệu nào trong phiên làm việc này.</p> : <div className="space-y-2">
                {recentDownloads.map((item) => <div
    key={item.id}
    onClick={() => onSelectDoc(item)}
    className="p-3 bg-slate-50 hover:bg-blue-50/60 rounded-xl border border-slate-100 transition-colors cursor-pointer space-y-1"
  >
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold px-1.5 py-0.5 bg-emerald-100 text-emerald-800 rounded">
                        Đã tải
                      </span>
                      <span className="text-[10px] text-slate-400">{item.fileType} • {item.fileSize}</span>
                    </div>
