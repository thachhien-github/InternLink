...
export const VersionHistoryTimeline = ({
  ...
  const defaultVersions: VersionItem[] = versions || [
    {
      ...
      summary:
        "Đã bổ sung sơ đồ ERD Payment Module theo đúng yêu cầu nhận xét của Giảng viên ở Lần 2.",
      duplicateScore: 4,
      fileSize: "3.2 MB",
      status: "approved",
      lecturerFeedback:
        "Báo cáo rất chi tiết, bổ sung sơ đồ chuẩn quy chuẩn. Đạt yêu cầu.",
    },
    {
      version: 2,
      ...
      lecturerFeedback:
        "Cần bổ sung sơ đồ ERD cho module Payment mới được tính điểm tối đa.",
    },
    {
      version: 1,
      ...
      status: "revision_requested",
      lecturerFeedback:
        "Nội dung quá ngắn, chưa thể hiện đủ công việc tuần này.",
    },
  ];
      ...
      <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
        {defaultVersions.map((v) => (
          <div key={v.version} className="relative group">
            {/* Timeline node icon */}
            <div
              className={`absolute -left-6 top-0 w-5 h-5 rounded-full border-2 bg-white flex items-center justify-center shrink-0 ${
 v.status === "approved"
 ...
 : "border-blue-600 text-blue-600"
            ...
            <div className="p-4 rounded-md border border-slate-200/80 bg-slate-50/50 hover:bg-blue-50/40 transition-colors space-y-2">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-200/60 text-xs">
                <div className="flex items-center gap-3">
                  <span className="text-[11px] text-slate-500">
                    File:{" "}
                    <strong className="text-slate-700">{v.fileSize}</strong>
                  </span>
                  <span
                    className={`text-[11px] font-bold ${v.duplicateScore > 20 ? "text-red-600" : "text-emerald-600"}`}
                  >
