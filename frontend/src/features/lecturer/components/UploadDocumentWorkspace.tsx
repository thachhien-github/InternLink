...
export const UploadDocumentWorkspace = ({
  ...
  const [major, setMajor] = useState(initialData?.major || "T\u1EA5t c\u1EA3 ng\xE0nh");
  const [description, setDescription] = useState(
    initialData?.description || "Bi\u1EC3u m\u1EABu chu\u1EA9n ban h\xE0nh theo quy \u0111\u1ECBnh c\u1EE7a Khoa C\xF4ng ngh\u1EC7 Th\xF4ng tin n\u0103m h\u1ECDc 2026."
  );
  const [uploadedFile, setUploadedFile] = useState(
    initialData ? { name: `${initialData.title}.${initialData.fileType.toLowerCase()}`, size: initialData.fileSize, type: initialData.fileType } : null
  );
  const [isDragging, setIsDragging] = useState(false);
  const [versionNumber, setVersionNumber] = useState(initialData?.version || "v2.0");
  const [releaseNote, setReleaseNote] = useState("B\u1ED5 sung ti\xEAu chu\u1EA9n nh\u1EADn di\u1EC7n m\u1EDBi, c\u1EADp nh\u1EADt ma tr\u1EADn thang \u0111i\u1EC3m 100.");
  ...
  const handleFileDrop = (e) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      triggerToast(`\u0110\xE3 \u0111\xEDnh k\xE8m t\u1EC7p: ${file.name}`);
    }
  };
  const handleSimulateSelectFile = () => {
    const mockFiles = [
      { name: "Mau_Bao_Cao_Thuc_Tap_2026.docx", size: "1.5 MB", type: "DOCX" },
      { name: "Quy_Che_Thuc_Tap_CNTT.pdf", size: "2.1 MB", type: "PDF" },
      { name: "Bang_Danh_Gia_Mentor.xlsx", size: "480 KB", type: "XLSX" }
    ];
    const picked = mockFiles[Math.floor(Math.random() * mockFiles.length)];
    setUploadedFile(picked);
    triggerToast(`\u0110\xE3 t\u1EA3i l\xEAn t\u1EC7p m\u1EABu: ${picked.name}`);
  };
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">
                {isEditing ? `C\u1EADp nh\u1EADt t\xE0i li\u1EC7u: ${initialData.title}` : "\u0110\u0103ng bi\u1EC3u m\u1EABu & T\xE0i li\u1EC7u m\u1EDBi"}
              </h1>
            </div>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Phát hành tài liệu, biểu mẫu và đề cương thực tập chuẩn hóa cho sinh viên toàn Khoa.
            </p>
          </div>
        </div>
  ...
  >
    ...
    /* Học kỳ */
            ...
            <div>
              <label className="block font-bold text-slate-800 mb-1">Học kỳ áp dụng</label>
              <select
    value={semester}
  ...
  >
    ...
    /* Mô tả chi tiết */
            ...
            <div className="md:col-span-2">
              <label className="block font-bold text-slate-800 mb-1">Mô tả ngắn &amp; Hướng dẫn sử dụng</label>
              <textarea
    rows={3}
    value={description}
  ...
  />
    ...
    className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all cursor-pointer ${isDragging ? "border-blue-600 bg-blue-50/80 scale-[1.01]" : "border-slate-300 bg-slate-50/50 hover:border-blue-400 hover:bg-slate-50"}`}
    onClick={handleSimulateSelectFile}
  >
            {uploadedFile ? <div className="max-w-md mx-auto bg-white p-4 rounded-2xl border border-blue-200 shadow-sm flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-blue-100 text-blue-700 font-black flex items-center justify-center text-xs">
                    {uploadedFile.type}
                  </div>
                  <div className="text-left">
                    ...
                    <p className="text-[10px] text-slate-500">{uploadedFile.size} • Đã sẵn sàng phát hành</p>
                  </div>
                </div>
                <button
    type="button"
  ...
  >
                ...
                <div>
                  <p className="font-extrabold text-slate-900 text-sm">
                    Kéo &amp; Thả tệp vào đây, hoặc <span className="text-blue-600 underline">duyệt tệp từ máy tính</span>
                  </p>
                  <p className="text-xs text-slate-400 mt-1 font-medium">Dung lượng tối đa: 25MB mỗi tệp</p>
                </div>
    ...
    /* Supported Formats */
                ...
                <div className="flex flex-wrap items-center justify-center gap-1.5 pt-2">
                  {["PDF", "DOCX", "XLSX", "ZIP", "RAR", "PPTX", "PNG", "JPG"].map((ext) => <span
    key={ext}
    className="px-2 py-0.5 bg-white text-slate-600 border border-slate-200 font-extrabold text-[10px] rounded-md shadow-2xs"
  >
                      {ext}
                    </span>)}
                </div>
    ...
    /* SECTION 3: VERSION NUMBER & RELEASE NOTES */
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div>
              <label className="block font-bold text-slate-800 mb-1">Số phiên bản (Version Number)</label>
              <input
    type="text"
    value={versionNumber}
    onChange={(e) => setVersionNumber(e.target.value)}
    placeholder="VD: v2.0, v2.1..."
    className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-blue-700"
  />
            </div>

            <div className="md:col-span-2">
              <label className="block font-bold text-slate-800 mb-1">Ghi chú thay đổi (Release Notes)</label>
              <input
    type="text"
    value={releaseNote}
    ...
    /* SECTION 4: VISIBILITY & PERMISSIONS */
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-4">
          ...
          <div className="space-y-3 text-xs">
            <label className="block font-bold text-slate-800">Đối tượng được phép xem &amp; tải tài liệu:</label>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {[
    { id: "T\u1EA5t c\u1EA3 sinh vi\xEAn", label: "T\u1EA5t c\u1EA3 sinh vi\xEAn", desc: "C\xF4ng khai cho to\xE0n Khoa" },
    { id: "Theo ng\xE0nh", label: "Theo ng\xE0nh", desc: "Ch\u1EC9 c\xE1c ng\xE0nh ch\u1EC9 \u0111\u1ECBnh" },
    { id: "Theo l\u1EDBp", label: "Theo l\u1EDBp", desc: "Ch\u1EC9 l\u1EDBp h\u1ECDc ph\u1EA7n c\u1EE5 th\u1EC3" },
    { id: "Theo h\u1ECDc k\u1EF3", label: "Theo h\u1ECDc k\u1EF3", desc: "Ch\u1EC9 sinh vi\xEAn HK I - 2026" }
  ].map((opt) => <div
    key={opt.id}
    onClick={() => setVisibility(opt.id)}
    className={`p-3 rounded-xl border cursor-pointer transition-all ${visibility === opt.id ? "bg-blue-50 border-blue-600 text-blue-900 shadow-xs" : "bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100"}`}
  >
  ...
  />
            ...
            {visibility === "Theo l\u1EDBp" && <div className="pt-2">
                <label className="block font-bold text-slate-800 mb-1">Mã lớp học phần áp dụng:</label>
                <input
    type="text"
    value={targetClass}
