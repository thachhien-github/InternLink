...
export const FilterBar = ({ filters, onFilterChange, onReset }) => {
  return (
    <div className="bg-white p-4 rounded-lg border border-slate-200/80 shadow-xs space-y-3 mb-5">
      {/* Search Input */}
      <div className="flex items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-xs bg-slate-50 border border-slate-200 rounded-md outline-none focus:border-blue-500 focus:bg-white transition-all text-slate-900 font-medium"
          />
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            onClick={onReset}
            className="px-3 py-2 text-xs font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-md border border-slate-200 transition-colors flex items-center gap-1.5"
            title="Xóa bộ lọc"
          >
          ...
          <button
            className="p-2 text-slate-600 hover:text-blue-600 hover:bg-slate-100 rounded-md border border-slate-200 transition-colors"
            title="Tùy chỉnh cột"
          >
