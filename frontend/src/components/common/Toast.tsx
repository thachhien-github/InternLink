...
const SingleToastCard = ({ item, index, onDismiss }: SingleToastCardProps) => {
  ...
  return (
    ...
    >
      <div
        className={`bg-white rounded-lg p-3.5 shadow-md shadow-slate-300/60 border ${config.borderColor} flex items-stretch gap-3 relative overflow-hidden`}
      >
        <div className={`w-1.5 rounded-full ${config.barBg} shrink-0 my-0.5`} />
        <div className="flex-1 pr-6 py-0.5 space-y-0.5 min-w-0">
          <h4
            className={`text-xs font-bold tracking-tight ${config.titleColor}`}
          >
            {config.title}
          </h4>
        ...
        <div
          className={`w-8 h-8 rounded-full ${config.iconBg} flex items-center justify-center shrink-0 self-center shadow-xs`}
        >
